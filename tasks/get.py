...
def get_deceases_by_year_by_sex_arg(upstream, product, causeCodes):
    ...
    df["sex_correspondence"] = df["sexo"].apply(
    ...
    df_causes_subset_all["deceases_subset_causes_indeterminado"] = df_causes_subset_all[
        "deceases_subset_causes_indeterminado"
    ].astype("int64")
    df_causes_subset_all["deceases_subset_causes_mujer"] = df_causes_subset_all[
        "deceases_subset_causes_mujer"
    ].astype("int64")
    df_causes_subset_all["deceases_subset_causes_varon"] = df_causes_subset_all[
        "deceases_subset_causes_varon"
    ].astype("int64")

    # 3. combine
    output_df = pandas.merge(
        df_deceases_all, df_causes_subset_all, on="year", how="left"
    )
