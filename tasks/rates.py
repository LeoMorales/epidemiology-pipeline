...
def get_age_grouping_rates(upstream, product, causesCodes):
    """
    ...
    for _year, df_year in df.groupby('year'):
        ...
        for _age_group, df_year_age_group in df_year.groupby('age_group'):
            # '0 - 5', '6 - 15', ..., '>85'
            df_year_age_group_ = df_year_age_group[df_year_age_group['codigo_defuncion'].isin(causesCodes)]
    ...
    rates_df = pandas.DataFrame(
        data,
        columns=['year', 'age_group', 'rate_1000'])
